...
pre_1996 <- raw_1996 |> 
  ...
  mutate(
    ...
    section = "Main",
    cut = num,
    harvesting_id = get_harvest_id(year = 1996,
                                   plot = plot,
                                   section = section,
                                   product = crop,
                                   cut = cut),
    harvestingloss_id = get_harvestingloss_id(year = 1996,
                                              plot = plot,
                                              section = section,
                                              coordinate = "X",
                                              product = crop,
...
tbl_1996 <- pre_1996 |> select(any_of(harvesting_cols))
supp_1996 <- pre_1996 |> select(any_of(supp_harvesting_cols))
