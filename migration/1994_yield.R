...
pre_1994 <- raw_1994 |> 
  ...
  mutate(
    ...
    section = "Main",
    cut = num,
    harvesting_id = get_harvest_id(year = 1994,
                                   plot = plot,
                                   section = section,
                                   product = crop,
                                   cut = cut),
    harvestingloss_id = get_harvestingloss_id(year = 1994,
                                              plot = plot,
                                              section = section,
                                              coordinate = "X",
                                              product = crop,
...
tbl_1994 <- pre_1994 |> select(any_of(harvesting_cols))
supp_1994 <- pre_1994 |> select(any_of(supp_harvesting_cols))
...
pre_1994_bio_pasture <- xl_pasture$massings |> 
  ...
  mutate(cut = row_number(),
         biomassing_id = get_biomassing_id(year = 1994,
                                           plot = plot,
                                           section = "main",
...
tbl_1994_harvests <- tbl_1994
supp_1994_harvests <- supp_1994

tbl_1994_loss <- bind_rows()
