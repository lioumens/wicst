...
pre_1993 <- raw_1993 |> 
  ...
  mutate(
    ...
    section = "Main",
    cut = num,
    harvesting_id = get_harvest_id(year = 1993,
                                   plot = plot,
                                   section = section,
                                   product = crop,
                                   cut = cut),
    harvestingloss_id = get_harvestingloss_id(year = 1993,
                                              plot = plot,
                                              section = section,
                                              coordinate = "X",
                                              product = crop,
...
tbl_1993 <- pre_1993 |> select(any_of(harvesting_cols))
supp_1993 <- pre_1993 |> select(any_of(supp_harvesting_cols)) |>
  filter(if_any(c(harvest_length, harvest_width, comments), \(x) !is.na(x)))

# no losses
# biomassings for pasture

# xl_pasture$massings |> names()
pre_1993_past <- xl_pasture$massings |> 
  filter(year == 1993, !is.na(yield), type == "quadrat") |>
  group_by(year, plot, subsample) |> 
  arrange(date) |> 
  mutate(cut = row_number(),
         biomassing_id = get_biomassing_id(year = 1993,
                                           plot = plot,
                                           section = "main",
...
tbl_1993_past <- pre_1993_past |> select(any_of(biomassing_cols))
supp_1993_past <- pre_1993_past |> select(any_of(supp_biomassing_cols))
  

# collect -----------------------------------------------------------------

tbl_1993_harvests <- tbl_1993
supp_1993_harvests <- supp_1993

tbl_1993_loss <- bind_rows()
supp_1993_loss <- bind_rows()

tbl_1993_bio <- bind_rows(
  tbl_1993_past
)

supp_1993_bio <- bind_rows(
  supp_1993_past
)
